...
class Menu():
    ...
    def executeMainOption(self,option):
        if option == 1:
            flash_mbps=self.myIntegerInput("Set Flash Crowd Mbps Throughput 10..250: ")
            while flash_mbps < 1 or flash_mbps > 250:
                flash_mbps=self.myIntegerInput("Set Flash Crowd Mbps Throughput 10..250: ")
            
            self.location = "Flash"
            self.flashAdvUdpFlooder = AdvancedUDPFlooder(self.dest_ip, self.dest_port, flash_mbps, self.ratio, name="Flash Crowd Traffic")
            self.flashAdvUdpFlooder.start()
            return
        if option == 2:
            attack_mbps=self.myIntegerInput("Set Attack Mbps Throughput 10..250: ")
            while attack_mbps < 1 or attack_mbps > 250:
                mbps=self.myIntegerInput("Set Attack Mbps Throughput 10..250: ")

            attack_ratio=self.myFloatInput("Set Attack CPS/PPS Ration 0.01..0.99: ")
            while attack_ratio < 0.01 or attack_ratio > 0.99:
                attack_ratio=self.myFloatInput("Set Attack CPS/PPS Ration 0.01..0.99: ")

            self.location = "Attack"
            self.atkAdvUdpFlooder = AdvancedUDPFlooder(self.dest_ip, self.dest_port, attack_mbps, attack_ratio, rand=False, name="Attack Traffic")
    ...
    def start(self):
        if len(sys.argv) < 5:
            print("Usage: [IP] [Port] [Mbps] [Ratio]\r\n")
            print("Positional Arguments: ")
            print(" - IP             Target Machine IP Address")
            print(" - Port           UDP Destination Port")
            print(" - Mbps           Expected Throughput, 1..250")
            print(" - Ratio          Expected CPS/PPS Ratio, 0.01..0.99")
            exit(1)
        
        self.dest_ip = sys.argv[1]
        self.dest_port = int(sys.argv[2])
        self.mbps = int(sys.argv[3])
        self.ratio = float(sys.argv[4])

        if self.dest_port < 1 or self.dest_port > 65535:
            print("Error: Port value not in range, valid range 1..65535")
            exit(1)

        if self.mbps < 10 or self.mbps > 250:
            print("Error: Mbps value not in range, valid range 10..250")
            exit(1)

        if self.ratio < 0.01 or self.ratio > 0.99:
            print("Error: Ratio value is not in range, valid range 0.01..0.99")
            exit(1)
        
        self.normalAdvUdpFlooder = AdvancedUDPFlooder(self.dest_ip, self.dest_port, self.mbps, self.ratio, name="Normal Traffic")
